...
const ArtistEditPage = () => {
    ...
    const deleteMusic = async (music) => {
        try {
            await axios.delete(`${API_URL}/admin/deleteMusic/${music._id}`, {
                headers: {
                    'authorization': `Bearer ${token}`,
                }
            })
    ...
    const deleteArtist = async () => {
        if (confirm('Do you want to delete this artist?')) {
            if (confirm('really!!!')) {
                ...
                try {
                    await axios.delete(`${API_URL}/admin/deleteArtist/${artist._id}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    })
                        ...
                        return (
                            <div className="w-full h-[25%] flex bg-white/10 hover:bg-black-100/40 transition-all duration-200 last:rounded-b-lg" key={i}>
                                <div className="h-full aspect-square p-2">
                                    <div className="bg-cover bg-center w-full h-full rounded-md" style={{ backgroundImage: `url(${API_URL}/musicImg/${encodeURIComponent(e.coverUrl)})` }}></div>
                                </div>
